    ...
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-transparent to-black/20">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative">
          <Card className="glass-dark border-[#00FF85]/30">
            <CardContent className="p-8">
              <div className="flex items-center justify-between mb-6">
                <Button
                  ...
                  size="icon"
                  onClick={prevCommand}
                  className="text-[#00FF85] hover:bg-[#00FF85]/10"
                >
                  <ChevronLeft className="h-6 w-6" />
                </Button>

                <div className="text-center flex-1">
                  <span className="inline-block px-3 py-1 bg-[#00FF85]/20 text-[#00FF85] rounded-full text-sm font-medium mb-4">
                    {commands[currentIndex].category}
                  </span>
                  <div className="bg-[#2f3136] rounded-lg p-4 font-mono">
                    <div className="text-[#00FF85] text-lg font-bold mb-2">{commands[currentIndex].command}</div>
                ...
                {commands.map((_, index) => (
                  <button
                    ...
                    className={`w-3 h-3 rounded-full transition-colors ${
                      index === currentIndex ? "bg-[#00FF85]" : "bg-gray-600"
                    }`}
                  />
                ))}
              </div>
